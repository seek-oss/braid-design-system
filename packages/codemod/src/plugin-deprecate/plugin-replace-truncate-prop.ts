  ...
  return {
    ...
    visitor: {
            ...
            if (
              ...
              for (const specifier of statement.node.specifiers) {
                if (
                  ...
                  this.deprecations.includes(specifier.imported.name)
                ) {
      ...
      JSXOpeningElement(path) {
        ...
        if (elementName && this.deprecations.includes(elementName)) {
          for (const attr of path.get('attributes')) {
            if (t.isJSXAttribute(attr.node)) {
              if (attr.node.name.name === 'truncate') {
                attr.replaceWith(
                  t.jsxAttribute(
                    t.jsxIdentifier('maxLines'),
