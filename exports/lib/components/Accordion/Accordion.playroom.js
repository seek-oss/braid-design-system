...
const _excluded = ['space', 'size', 'tone'];
...
const spaceScale = ['none'].concat(
  _toConsumableArray(Object.keys(wireframe.space.space)),
);

const filterSpace = function filterSpace(space) {
  const filteredSpace = mapResponsiveValue(space, function (value) {
    if (spaceScale.includes(value) && !validSpaceValues.includes(value)) {
      throw new Error(
        "To ensure adequate space for touch targets, 'space' prop values must be one of the following: ".concat(
          validSpaceValues
            .map(function (x) {
              return '"'.concat(x, '"');
            })
