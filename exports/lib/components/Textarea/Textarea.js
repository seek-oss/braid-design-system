...
const renderCount = function renderCount(_ref) {
  const characterLimit = _ref.characterLimit,
...
const pxToInt = function pxToInt(str) {
  return typeof str === 'string' ? parseInt(str.replace('px', ''), 10) : 0;
};

const calculateLines = function calculateLines(target, lines, lineLimit) {
  const _window$getComputedSt = window.getComputedStyle(target),
...
export var Textarea = /* #__PURE__*/ forwardRef(function (_ref2, ref) {
  const value = _ref2.value,
    ...
    highlightRangesProp =
      _ref2$highlightRanges === void 0 ? [] : _ref2$highlightRanges,
  ...
  const _useState = useState(lines),
  ...
  const excessCharactersRange =
    characterLimit && inputLength > characterLimit
      ...
      : [];
  const highlightRanges = [].concat(
    excessCharactersRange,
    _toConsumableArray(highlightRangesProp),
  );
