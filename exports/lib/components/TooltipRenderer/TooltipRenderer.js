...
const StaticTooltipContext = /* #__PURE__*/ createContext(false);
...
export var TooltipTextDefaultsProvider = function TooltipTextDefaultsProvider(
  _ref2,
) {
...
export var TooltipRenderer = function TooltipRenderer(_ref4) {
  ...
  assert(
    ...
    "The 'placement' prop must be one of the following: ".concat(
      validPlacements.join(', '),
    ),
  );
  ...
  const _usePopperTooltip = usePopperTooltip(
      {
        placement,
  ...
  useEffect(
    function () {
      if (visible) {
        const handleKeyDown = function handleKeyDown(_ref5) {
          const key = _ref5.key;
  ...
  useEffect(
    function () {
      ...
      if (visible) {
        const handleFocusIn = function handleFocusIn(event) {
