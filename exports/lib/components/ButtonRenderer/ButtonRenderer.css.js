...
let _selectors2, _selectors3, _selectors5, _selectors6;

import * as __vanilla_filescope__ from '@vanilla-extract/css/fileScope';

__vanilla_filescope__.setFileScope(
  'lib/components/ButtonRenderer/ButtonRenderer.css.ts',
  'braid-design-system',
);
...
export var activeOverlay = style(
  {
    selectors:
      ((_selectors2 = {}),
      _defineProperty(_selectors2, ''.concat(root, ':active &'), {
        opacity: 1,
      }),
...
export var hoverOverlay = style(
  {
    selectors:
      ...
      _defineProperty(
        _selectors3,
        ''.concat(root, ':hover:not(:disabled):not(:active) &'),
        {
          opacity: 1,
        },
      ),
      _defineProperty(
        _selectors3,
        ''.concat(lightHoverBg, ':hover:not(:disabled):not(:active) &'),
        {
          opacity: 0.075,
        },
      ),
      _defineProperty(
        ...
        ''
          .concat(lightHoverBg)
          .concat(inverted, ':hover:not(:disabled):not(:active) &'),
        {
          opacity: 0.15,
        },
      ),
...
const stylesForBreakpoint = function stylesForBreakpoint(breakpoint, size) {
  const height =
    size === 'small'
      ? calc.add(
...
export var loadingDot = style(
  {
    ...
    selectors:
      ((_selectors6 = {}),
      _defineProperty(_selectors6, '&:nth-child(1)', {
        animationName: dot1,
      }),
