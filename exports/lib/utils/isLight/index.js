...
const getLinearGradientColors = function getLinearGradientColors(color) {
...
export var isLight = function isLight(inputColor) {
  ...
  return colors.some(function (color) {
    const _parseToRgb = parseToRgb(color),
        red = _parseToRgb.red,
    ...
    const yiq = (red * 299 + green * 587 + blue * 114) / 1000; // Colour is considered `light` if greater than the midpoint:
    // eg. 256 / 2 = 128.
