...
const AA_GRAPHIC_CONTRAST = 3; // http://www.w3.org/TR/UNDERSTANDING-WCAG20/visual-audio-contrast-contrast.html#key-terms
...
export function findClosestAccessibleLighterColor(inputColor, fixedColor) {
  const contrastRatio = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : AA_TEXT_CONTRAST;
  const normalisedInput = toColorString(parseToRgb(inputColor));
  const normalisedFixed = toColorString(parseToRgb(fixedColor));

  const _parseToHsl = parseToHsl(normalisedInput),
      hue = _parseToHsl.hue,
...
export function findClosestAccessibleDarkerColor(inputColor, fixedColor) {
  ...
  const _parseToHsl2 = parseToHsl(normalisedInput),
...
const smoothSaturation = function smoothSaturation(saturation, luminance) {
...
const smoothLightness = function smoothLightness(lightness, luminance) {
  const isBright = luminance > 0.6;
...
export function getLightVariant(color) {
  const _parseToHsl3 = parseToHsl(color),
