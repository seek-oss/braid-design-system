...
var import_workerpool = __toModule(require_src());
var import_cli_progress = __toModule(require_cli_progress());
var import_chalk = __toModule(require_source());
if (process.argv.length < 4) {
  console.error(import_chalk.default.red(`Requires a \`version\` and a \`glob\` to run against.
...
$> yarn braid-upgrade v31 "**/*.{ts,tsx}"
  ...
  const paths = import_fast_glob.default.sync(pathGlob, {
  ...
  const progress = new import_cli_progress.default.SingleBar({
    format: (options, params) => {
      ...
      const bar = barCompleteString.substr(0, completeSize) + barIncompleteString.substr(0, incompleteSize);
      return `[${bar}] ${params.value}/${params.total}`;
    },
    hideCursor: true,
    barCompleteChar: "\u25A0",
    barIncompleteChar: "-"
  }, import_cli_progress.default.Presets.shades_classic).on("SIGINT", () => {
    progress.stop();
    process.exitCode = 1;
  });
