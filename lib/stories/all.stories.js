...
var webFontLinkTags = uniq(flatten(values(themes).map(function (theme) {
...
var getComponentName = function getComponentName(filename) {
  var matches = filename.match(/([a-zA-Z]+)(?:\.docs|\.screenshots)\.tsx?$/);
...
var RenderExample = function RenderExample(_ref2) {
  ...
  var _useSourceFromExample = useSourceFromExample('id', example),
...
var storiesFromScreenshots = require.context('../components', true, /\.screenshots\.tsx?$/);
...
Object.keys(allStories).sort(function (a, b) {
  ...
  var stories = storiesOf(componentName, module);
  var docs = allStories[componentName];
  var storyThemes = values(themes).filter(function (theme) {
    if (theme.name === 'docs') {
      return false;
    }
  ...
  storyThemes.forEach(function (theme) {
    ...
    var renderStory = function renderStory() {
