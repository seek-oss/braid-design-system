...
var MENU_TRIGGER_UP = actionTypes.MENU_TRIGGER_UP,
...
var isDivider = function isDivider(node) {
  return _typeof(node) === 'object' && node !== null && 'type' in node && node.type === MenuItemDivider;
};
...
export var MenuRenderer = function MenuRenderer(_ref) {
  ...
  assert(items.every(function (item) {
    return _typeof(item) === 'object' && 'type' in item && (item.type === MenuItem || item.type === MenuItemCheckbox || item.type === MenuItemLink || item.type === MenuItemDivider);
  ...
  var onTriggerKeyUp = function onTriggerKeyUp(event) {
    var targetKey = normalizeKey(event); // Space key in keyup/keydown handler in Firefox triggers a click event.
  ...
  var dividerCount = 0;
  return /*#__PURE__*/_jsx(MenuRendererContext.Provider, {
    value: true
    ...
    var menuItemIndex = i - dividerCount;
    return /*#__PURE__*/_jsx(MenuRendererItemContext.Provider, {
      value: {
  ...
  })), _Overlay || (_Overlay = /*#__PURE__*/_jsx(Overlay, {
    boxShadow: "borderStandard",
    borderRadius: "standard",
    visible: true
  })))), open ? /*#__PURE__*/_jsx(Box, {
    onClick: function onClick(event) {
      event.stopPropagation();
      event.preventDefault();
      dispatch({
