...
import { negativeMarginLeft, negativeMarginTop } from '../../atoms/negativeMargin/negativeMargin';
...
export var Inline = function Inline(_ref) {
  ...
  assert(validInlineComponents.includes(component), "Invalid Inline component: '".concat(component, "'. Should be one of [").concat(validInlineComponents.map(function (c) {
    return "'".concat(c, "'");
  }).join(', '), "]"));
  var isList = component === 'ol' || component === 'ul';
  var inlineItemComponent = isList ? 'li' : 'div';
  ...
  }, data ? buildDataAttributes(data) : undefined), /*#__PURE__*/React.createElement(Box, _extends({
    component: component,
    className: negativeMarginLeft(space),
    flexWrap: "wrap"
  }, collapsibleAlignmentProps), Children.map(orderChildren(flattenChildren(children)), function (child) {
    return child !== null && child !== undefined ? /*#__PURE__*/React.createElement(Box, _extends({
