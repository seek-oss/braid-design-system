...
export var Hidden = function Hidden(_ref) {
  ...
  if (process.env.NODE_ENV === 'development' && screen) {
    ...
    console.warn("You used the \"screen\" prop on Hidden, but this probably doesn't do what you expect. If you're trying to provide content to screen readers without rendering it to the screen, use the <HiddenVisually> component instead. The \"screen\" prop is likely to be deprecated in a future release.");
  }

  var inText = Boolean(useContext(TextContext));
  var inHeading = Boolean(useContext(HeadingContext));
  var hiddenOnScreen = Boolean(screen);
  var hiddenOnPrint = Boolean(print);

  var _resolveResponsiveRan = resolveResponsiveRangeProps({
    above: above,
    below: below
  }),
  ...
  return /*#__PURE__*/React.createElement(Box, _extends({
    display: hiddenOnScreen ? 'none' : [hiddenOnMobile ? 'none' : display, hiddenOnTablet ? 'none' : display, hiddenOnDesktop ? 'none' : display],
