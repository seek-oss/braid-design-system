...
import { useTextTone, useWeight, touchableText, useText } from '../../hooks/typography';
...
export var PrivateTextLinkRenderer = function PrivateTextLinkRenderer(props) {
  ...
  assert(function () {
    ...
    var inActions = actionsContext !== null;
...
function useDefaultLinkWeight() {
  ...
  var inHeading = useContext(HeadingContext);
  var textContext = useContext(TextContext);
  var hasPlainBackground = backgroundContext === 'body' || backgroundContext === 'card' || backgroundContext === 'neutralLight';
  var inPlainText = !textContext || textContext.tone === undefined || textContext.tone === 'neutral' || textContext.tone === 'secondary';
  return hasPlainBackground && (inHeading || inPlainText) ? 'regular' : 'weak';
}

function useLinkStyles(weight, showVisited) {
  var inHeading = useContext(HeadingContext);
  var mediumWeight = useWeight('medium');
  var linkTone = useTextTone({
    tone: 'link'
  });
  return [weight === 'weak' ? styles.underlineAlways : [linkTone, styles.underlineOnHoverOnly], !inHeading && weight !== 'weak' ? mediumWeight : null, showVisited ? styles.visited : null];
}
...
function InlineLink(_ref) {
  ...
  var defaultWeight = useDefaultLinkWeight();
  var weight = weightProp !== null && weightProp !== void 0 ? weightProp : defaultWeight;
  return /*#__PURE__*/_jsx(TextLinkRendererContext.Provider, {
    value: weight
  }, void 0, children({
    style: {},
    className: clsx(useLinkStyles(weight, showVisited), reset !== false ? atoms({
...
function ButtonLink(_ref2) {
  ...
  var textLinkWeight = useDefaultLinkWeight();
  var tone = textLinkWeight === 'weak' ? 'neutral' : 'link';
  var buttonLinkTextProps = {
    size: size,
    tone: tone,
    baseline: false
  };
  ...
  return /*#__PURE__*/_jsx(Box, {
    position: "relative"
  }, void 0, /*#__PURE__*/_jsx(TextLinkRendererContext.Provider, {
    value: textLinkWeight
  }, void 0, /*#__PURE__*/_jsx(TextContext.Provider, {
    value: buttonLinkTextProps
  }, void 0, children({
    style: {},
    className: clsx(styles.button, useLinkStyles(textLinkWeight, showVisited), useText(buttonLinkTextProps), size === 'standard' ? touchableText.standard : null, reset !== false ? atoms({
      reset: typeof reset === 'string' ? reset : 'a'
    }) : null, atoms({
      cursor: 'pointer',
      outline: 'none',
