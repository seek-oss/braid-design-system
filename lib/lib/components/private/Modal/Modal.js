...
var ModalContext = /*#__PURE__*/createContext(false);
...
var ModalPortal = function ModalPortal(_ref) {
  ...
  var alreadyInModalContext = useContext(ModalContext);
  assert(!alreadyInModalContext, 'Nested modals are not supported.');
  useEffect(function () {
    var modalContainerId = 'braid-modal-container';
    var element = document.getElementById(modalContainerId);
...
var OPEN_MODAL = 1;
var CLOSE_MODAL = 2;
var ANIMATION_COMPLETE = 3; // States
...
export var Modal = function Modal(_ref2) {
  ...
  var _useReducer = useReducer(reducer, INITIAL),
      _useReducer2 = _slicedToArray(_useReducer, 2),
      state = _useReducer2[0],
      dispatch = _useReducer2[1];

  var allowClose = useContext(AllowCloseContext);
  var shouldFocus = typeof document !== 'undefined' && typeof document.hasFocus === 'function' && document.hasFocus();
  var openRef = useRef(open);
  var modalRef = useRef(null);
  var headingRef = useRef(null);
  var closeHandlerRef = useRef(onClose);

  var initiateClose = function initiateClose() {
    if (allowClose) {
  ...
  var shouldAriaHideOthers = state === OPEN || state === CLOSING;
  useEffect(function () {
  ...
  useEffect(function () {
    var event = trapActive ? 'blur' : 'focus';
