...
__vanilla_filescope__.setFileScope("src/components/private/Modal/Modal.css.ts", "braid-design-system");
...
var easeOut = 'cubic-bezier(0.4, 0, 0, 1)';
...
var CLOSE_ICON_GUTTER_RATIO = 0.3;

var calculateIconCrop = function calculateIconCrop(level, breakpoint) {
  var size = calc.multiply(vars.headingLevel[level][breakpoint].capHeight, calc.add(1, CLOSE_ICON_GUTTER_RATIO));
  var offset = calc.divide(calc.multiply(size, CLOSE_ICON_GUTTER_RATIO), 2);
  var nudge = '1px';
  return {
...
var calculateCloseIconSize = function calculateCloseIconSize(level, breakpoint) {
  var size = calc.multiply(vars.headingLevel[level][breakpoint].capHeight, calc.add(1, CLOSE_ICON_GUTTER_RATIO));
  return {
    width: size,
    height: size
