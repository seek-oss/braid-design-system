...
var _excluded = ["children", "size", "tone", "space", "type", "start", "data"];
import React, { Children } from 'react';
import { Text } from '../Text/Text';
import { Stack } from '../Stack/Stack';
import { Box } from '../Box/Box';
import flattenChildren from 'react-keyed-flatten-children';
import { DefaultTextPropsProvider, useDefaultTextProps } from '../private/defaultTextProps';
import { lineHeightContainer } from '../../atoms/lineHeightContainer.css';
import * as styles from './List.css';

function numberToAlpha(inputNumber) {
  var returnValue = '';
  var counter = inputNumber;
...
function numberToRomanNumerals(inputNumber) {
  ...
  Object.keys(romanNumerals).forEach(function (romanNumeral) {
    var value = romanNumerals[romanNumeral];
...
var CharacterBullet = function CharacterBullet(_ref) {
  var _styles$minCharacterW;
  ...
  return /*#__PURE__*/_jsx(Box, {
    display: "inlineBlock",
    className: [(_styles$minCharacterW = styles.minCharacterWidth[length - 1]) !== null && _styles$minCharacterW !== void 0 ? _styles$minCharacterW : styles.minCharacterWidth[styles.minCharacterWidth.length - 1], styles.trimGutter]
  }, void 0, children, ".");
};
...
export var List = function List(_ref2) {
  ...
  var listItems = flattenChildren(children);
  var lastNumberLength = type === 'number' ? (listItems.length + (start - 1)).toString().length : -1;
  return /*#__PURE__*/_jsx(DefaultTextPropsProvider, {
    ...
    space: space,
    data: data
  }, void 0, Children.map(listItems, function (listItem, index) {
    var _CharacterBullet;

    var resolvedIndex = index + (start - 1);
    return /*#__PURE__*/_jsx(Box, {
      display: "flex"
      ...
      if (type === 'number') {
        return _CharacterBullet || (_CharacterBullet = /*#__PURE__*/_jsx(CharacterBullet, {
          length: lastNumberLength
        }, void 0, resolvedIndex + 1));
      }

      if (type === 'alpha') {
        return /*#__PURE__*/_jsx(CharacterBullet, {}, void 0, numberToAlpha(resolvedIndex + 1));
      }

      if (type === 'roman') {
        return /*#__PURE__*/_jsx(CharacterBullet, {
          length: 2
        }, void 0, numberToRomanNumerals(resolvedIndex + 1));
      }
