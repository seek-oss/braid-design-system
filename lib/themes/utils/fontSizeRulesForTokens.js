...
export default ({ tokens }) => {
  ...
  Object.keys(tokens.text).forEach(typeName => {
    ...
    rules.push(
      // Create fontSize atoms for each type
      ...
      // Create baseline override atoms for each type
      createResponsiveRules({
        tokens,
        selector: `.${typeName}.centered`,
        mobileRules: { transform: 'none' },
        desktopRules: { transform: 'none' }
      })
    );
    ...
    if (typeName === 'standard') {
      ...
      rules.push(
        createResponsiveRules({
          ...
          mobileRules: {
            fontSize: px(type.mobile.size),
            lineHeight: px(type.mobile.rows * tokens.rowHeight),
            paddingTop: mobileInteractionPadding,
            paddingBottom: mobileInteractionPadding
          },
          desktopRules: {
            fontSize: px(type.desktop.size),
            lineHeight: px(type.desktop.rows * tokens.rowHeight),
